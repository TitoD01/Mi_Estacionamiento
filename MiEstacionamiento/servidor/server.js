...
app.post('/registrarVehiculo', (req, res) => {
    const { patente, descripcion_modelo, anno, marca_id_marca, cliente_rut_cli } = req.body;
    const query = 'INSERT INTO vehiculo (patente, descripcion_modelo, anno, cliente_rut_cli,marca_id_marca) VALUES (?, ?, ?, ?, ?)';
    db.query(query, [patente, descripcion_modelo, anno, marca_id_marca, cliente_rut_cli], (err, result) => {
        if (err) {
...
app.get('/marcas', (req, res) => {
    const query = 'SELECT descripcion FROM marca';
    ...
    db.query(query, (err, result) => {
        if (err) {
            ...
            const marcas = result.map((row) => row.descripcion);
            res.json(marcas);
        }
    });
});
